...
internal object VelocityAdvancedJumpReset : VelocityMode("AdvancedJumpReset") {
    ...
    private object PauseSettings : ToggleableConfigurable(ModuleVelocity, "PauseConditions", true) {
        val onFlag by boolean("PauseOnFlag", true)
        val flagDelay by int("FlagDelay", 2, 1..20)
        val notCombating by boolean("PauseNotCombating", true)
        val combatTick by int("CombatTick", 8, 1..100)
    }

    private object AutoDisableSettings : ToggleableConfigurable(ModuleVelocity, "AutoDisable", true) {
        val onFlag by boolean("DisableOnFlag", true)
    }
    ...
    private val packetHandler = handler<PacketEvent> { event ->
        when (event.packet) {
            is PlayerPositionLookS2CPacket -> {
                flagTicks = 0
                if (AutoDisableSettings.onFlag) {
                    ModuleVelocity.enabled = false
                }
            }
