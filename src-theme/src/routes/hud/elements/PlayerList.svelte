<script lang="ts">
    ...
    const calculateLayout = (players: { name: string | TTextComponent }[]) => {
        ...
        for (const [index, player] of players.entries()) {
            ...
            columnWidths[col] = Math.max(columnWidths[col],
                getTextWidth(
                    nameStr,
                    "normal 500 20px 'Inter', system-ui, sans-serif") + buffer, 360
            );
        }
    };
    ...
    listen("moduleToggle", (e: ModuleToggleEvent) => {
        if (e.moduleName === "KeepTabList") {
            keepTabList = e.enabled;
        }
    });
    ...
    async function refreshSettings() {
        const cfg = await getModuleSettings("BetterTab");
        setVisibilityKeywords(cfg);
    }

</script>


{#if visible || keepTabList}
    <div class="tab-overlay" style="transform: scale({settings.scale});" transition:scale={{duration:300}}>
        {#if overlayPlayList}
            <div class="tab-container hud-container">
                ...
                {#if overlayPlayList.header && isVisible("Header")}
                    <div class="tab-header">
                        <TextComponent fontSize={20} allowPreformatting={true} preFormattingMonospace={false} textComponent={overlayPlayList.header}/>
                    </div>
                {/if}
...
<style lang="scss">
  ...
  .player-entry.friend {
    background-color: rgba(125, 255, 255, 0.3);
    filter: drop-shadow(0 0 4px rgba(125, 255, 255, 0.45));
  }

  .player-entry.staff {
    background-color: rgba(255, 125, 125, 0.3);
    filter: drop-shadow(0 0 4px rgba(255, 125, 125, 0.45));
  }
</style>
