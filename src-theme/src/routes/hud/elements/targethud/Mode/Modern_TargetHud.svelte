<script lang="ts">
    ...
    import {popIn, popOut} from "../../../../../util/animate_utils";
                ...
                <div class="main-wrapper">
                    ...
                    <div class="armor-stats">
                        {#if target.offHandStack?.identifier && !target.offHandStack.identifier.includes('air')}
                            <ItemStackDisplay stack={target.offHandStack}/>
                        {/if}
                        {#if target.mainHandStack?.identifier && !target.mainHandStack.identifier.includes('air')}
                            <ItemStackDisplay stack={target.mainHandStack}/>
                        {/if}
                        {#if target.armorItems[3].count > 0}
                            <ItemStackDisplay stack={target.armorItems[3]}/>
                        {/if}
                        {#if target.armorItems[2].count > 0}
                            <ItemStackDisplay stack={target.armorItems[2]}/>
                        {/if}
                        {#if target.armorItems[1].count > 0}
                            <ItemStackDisplay stack={target.armorItems[1]}/>
                        {/if}
                        {#if target.armorItems[0].count > 0}
                            <ItemStackDisplay stack={target.armorItems[0]}/>
                        {/if}
                    </div>
                </div>
...
<style lang="scss">
  ...
  .avatar {
    ...
    .avatar-inner {
      transform: scale(0.8);
      filter: drop-shadow(0 4 12px rgba($base, 0.85));
    }
  ...
  .name {
    ...
    text-shadow: 0 0 4px rgba(var(--text-color), 0.9);
    align-self: start;
    letter-spacing: 1px;
    display: flex;
    align-items: center;
