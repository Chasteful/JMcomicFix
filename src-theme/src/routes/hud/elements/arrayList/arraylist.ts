...
export async function getPrefixAsync(name: string) {
    ...
    const checkSetting = (setting: any): string | null => {
        const valueType = setting.valueType || setting.type;
        ...
        switch (valueType) {
            ...
            case 'MULTI_CHOOSE':
                const multiValue = (setting as MultiChooseSetting).value;
                return Array.isArray(multiValue) ? multiValue.join(', ') : multiValue;
            case 'INT_RANGE':
...
export function colorInterpolate(a: RGBColor, b: RGBColor, t: number): RGBColor {
    ...
    const [red, green, blue] = interpolated.coords
