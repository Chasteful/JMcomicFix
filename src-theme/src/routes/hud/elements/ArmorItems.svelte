<script lang="ts">
    ...
    import type {ClientPlayerDataEvent, ClientPlayerInventoryEvent, PlayerInventory} from "../../../integration/events";
...
<div class="armoritems-hud hud-container" id="armoritemshud" style="transform: scale({settings.scale});"
    ...
    <div class="armor-items">
        {#each [...armorSlots].reverse() as stack (stack)}
            {#if shouldShowSlot(stack)}
                <ItemStackDisplay
                        {stack}
                        showDurability={stack.maxDamage > 0}
                        durabilityPosition="outside"
                />
            {:else}
